...
(function() {
    ...
    if (typeof auth === 'undefined' || typeof db === 'undefined' || typeof firebase === 'undefined') {
        console.error('Firebase not initialised. Make sure Firebase scripts load before income.js');
        return;
    }
    ...
    auth.onAuthStateChanged(async (user) => {
        ...
        // Update every minute to refresh pending amounts and progress bars
    ...
    function renderActive(investments) {
            ...
            html += `
                <div class="plan-card" data-id="${inv.id}">
                    <div class="plan-header">
                        <span class="plan-name">${escapeHtml(inv.planName)}</span>
        ...
        document.querySelectorAll('.claim-btn.active').forEach(btn => {
            btn.addEventListener('click', async (e) => {
                ...
                await claimDailyIncome(invId, dailyAmount, btn);
            });
        });
    }

    // Claim daily income for an investment (runs in a transaction)
    async function claimDailyIncome(investmentId, dailyAmount, buttonElement) {
        ...
        // Disable the clicked button and store original text
