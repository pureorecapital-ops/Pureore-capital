...
(function() {
    ...
    if (typeof auth === 'undefined' || typeof db === 'undefined' || typeof firebase === 'undefined') {
        console.error('Firebase not initialised. Make sure Firebase scripts load before loan.js');
        return;
    }
    ...
    // Helper: show toast message (uses existing showToast if available, otherwise defines it)
    ...
    async function checkEligibility(userId) {
        ...
        if (!referralCode) {
            return { eligible: false, reason: '❌ Referral code missing. Please contact support.' };
        }
    ...
    async function showRestrictionAlert(userId) {
        ...
        if (restricted) {
            if (alertContainer) {
                ...
                `;
                const talkBtn = document.getElementById('talkToAdminBtn');
                if (talkBtn) {
                    talkBtn.addEventListener('click', () => window.open('https://wa.me/260951359357', '_blank'));
                }
            }
    ...
    // Request loan submission (with loading state)
    async function requestLoan(loanAmount, userId, requestBtn, loanMessage) {
    ...
    async function loadActiveLoans(userId) {
        ...
        // Attach repay handlers with loading state
        document.querySelectorAll('.repay-btn').forEach(btn => {
            btn.removeEventListener('click', repayHandler);
            btn.addEventListener('click', repayHandler);
        });
        async function repayHandler(e) {
    ...
    // Repay loan function (no loading state – handled by caller)
    async function repayLoan(loanId, repayAmount) {
        ...
        try {
            ...
            window.showToast('✅ Loan repaid successfully!', false);
            await loadActiveLoans(user.uid);
        } catch (err) {
            window.showToast(err.message, true);
            throw err; // rethrow so caller can re-enable button if needed
        }
    }
    ...
    auth.onAuthStateChanged(async (user) => {
        ...
        if (paybackInput) {
            paybackInput.addEventListener('input', () => {
                ...
                if (!isNaN(val) && val >= 6000) {
                    ...
                    const resultDiv = document.getElementById('paybackResult');
                    if (resultDiv && (!loanAmountInput || !loanAmountInput.value)) resultDiv.innerHTML = '';
                }
            });
        }
        ...
        if (requestBtn) {
            requestBtn.addEventListener('click', async () => {
                const loanAmount = parseFloat(loanAmountInput ? loanAmountInput.value : '');
                if (isNaN(loanAmount) || loanAmount < 6000 || loanAmount > 100000 || loanAmount % 6000 !== 0) {
                ...
                if (!eligibility.eligible) {
                    if (loanMessage) loanMessage.innerHTML = `<div class="eligibility-fail">${escapeHtml(eligibility.reason)}</div>`;
                    return;
                }
                ...
                const originalText = requestBtn.innerText;
                requestBtn.disabled = true;
                requestBtn.innerText = 'Submitting...';

                try {
                    await requestLoan(loanAmount, userId, requestBtn, loanMessage);
                    if (loanMessage) loanMessage.innerHTML = '<div style="color:#2ecc71; margin-top:0.5rem;">✅ Loan request submitted! Admin will review.</div>';
                    ...
                    if (loanMessage) loanMessage.innerHTML = `<div class="eligibility-fail">Error: ${escapeHtml(err.message)}</div>`;
                } finally {
                    requestBtn.disabled = false;
                    requestBtn.innerText = originalText;
                }
            });
        }

        // Bottom navigation – add listener only if not already handled in HTML
        const navItems = document.querySelectorAll('.nav-item');
        if (navItems.length && !navItems[0].hasAttribute('data-listener')) {
